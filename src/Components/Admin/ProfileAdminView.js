...
const ProfileAdminView = (props) => {
    ...
    const handleSubmit=(e)=>{
        e.preventDefault();
        if(update.userID!=="" && update.name!=="" && update.email!=="" && update.phoneNumber!=="" && update.password!==""&& update.dob!==""&& update.gender!==""&& update.role!==""&& update.verified!=="")
        {
            console.log(update)
            axios.post(`http://127.0.0.1:8000/api/profileAdmin`,update)
            .then(resp=>{
              history.push('/profileAdminview')
          }).catch(err=>{
            console.log(err);
          });
          e.preventDefault();
    ...
    return (
        <div>
          ...
          <form onSubmit={handleSubmit}>
            <div class="mb-6">
                <label for="formGroupExampleInput" className="form-label">User ID:</label>
            ...
            <div className="mb-3">
                ...
                <input type="text" name="name" className="form-control" id="formGroupExampleInput" defaultValue={name} onChange={handleChange}/>
              ...
              <div className="mb-3">
                ...
                <input type="text" name="role" className="form-control" id="formGroupExampleInput" defaultValue={role} onChange={handleChange} onMouseOver={mount}/>
                {/* <p style={{color: "red"}}>{shortdescErr}</p> */}
              </div>
              <div className="mb-3">
                <label for="formGroupExampleInput" className="form-label" style={{marginTop: "20px"}}> Verified:</label>
                <input type="text" className="form-control" id="exampleFormControlTextarea1" rows="3" name="verified" defaultValue={verified} onChange={handleChange}/>
                {/* <p style={{color: "red"}}>{descErr}</p> */}
              </div>
              ...
              <input type="submit" name="submit" value="Edit" className="btn btn-primary login_button"/>
          </form>  
        </div>
        </div>
    );
